...
var llStep = 10;
var llBase = 'http://kynaleski.com/wp-json/custom/lazyloadnews?page=';
var llPage = 1;
...
function lazyLoad(){
	...
	if(llPage !== 1){
		var llPages = $('#pagination-group-1').data('pages');
		if( llPage <= llPages){
			lazyGet(llPage);
		}else{
			console.log('lazyLoadComplete');
		}
	} else{
		lazyGet(llPage);
	}
...
function lazyGet( page ){

	var llUrl = llBase + page;
	...
	.done(function(data) {
		console.log("lazyGet success");
		lazyRender(data);
	})
	.fail(function() {
		console.log("lazyGet error");
	})
	.always(function() {
		console.log("lazyGet complete");
	});

}


function lazyRender(content){
	$('#llContainer').append(content);
	llPage++;
	setTimeout(function() {
		lazyLoad();
	}, 500);
